...
fetch_credentials() {
    ...
    spaces="\        "
    flag="-enable-headless-services"
    # sed will insert -enable-headless-services in the deployment generated by
    # link, at line 135, corresponding to the container args field in the svc
    # mirror deployment.
    sed_args="135i$spaces- $flag"
    # shellcheck disable=SC2001  
    echo "$($LINKERD --context="k3d-$cluster" multicluster link \
            --cluster-name="$cluster" \
            --log-level="debug" \
            --api-server-address="https://${lb_ip}:6443")" | sed "$sed_args" 
}
